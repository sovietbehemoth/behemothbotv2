...
async function permissions(message:any): Promise<void> {
    ...
    for (let i = 0; i < command.length; i++) {
        if (command[i] === "-q" && command[i+1] === "@me") {
            ...
            for (let i2 = 0; i2 < Bot.Permissions.length; i2++) {
                if (Bot.Permissions[i2].id === message.author.id) {
                    if (Bot.Permissions[i2].role === "developer") suf = true;
                }
            }
            
            if (suf === false) {
                Bot.Client.message.send(`BehemothBotDiscordManager: Insufficient permissions, expected (developer)`, message.channel_id);
            }
            ...
            await Deno.writeTextFile("./data/perm.json", JSON.stringify(Bot.Permissions));
        }
    }
}
